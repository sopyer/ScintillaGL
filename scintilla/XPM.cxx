...
void XPM::PixelAt(int x, int y, Colour &colour, bool &transparent) const {
...
void RGBAImage::SetPixel(int x, int y, Colour colour, int alpha) {
	unsigned char *pixel = &pixelBytes[0] + (y*width+x) * 4;
	// RGBA
	pixel[0] = GetRed(colour);
	pixel[1] = GetGreen(colour);
	pixel[2] = GetBlue(colour);
	pixel[3] = alpha;
}
